...
function parse(S, options) {
	...
	function parseName() {
		...
		if (options.removeNameSpace) {
			return removeNameSpace(S.slice(start, pos));
		} else {
	...
	if (options.simplify) {
		out = simplify(Array.isArray(out) ? out : [out]);
	}

	if (options.simplifyLostLess) {
		out = simplifyLostLess(Array.isArray(out) ? out : [out]);
	}

	if (options.logicalSimplify) {
		out = logicalSimplify(Array.isArray(out) ? out : [out]);
	}
