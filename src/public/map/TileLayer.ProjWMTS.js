...
L.TileLayer.ProjWMTS = L.TileLayer.extend({
	...
	_getTilesInfo: function (coords) {
		...
		result.size = { x: Math.abs(tilecolse - tilecolnw) + 1, y: Math.abs(tilerowse - tilerownw) + 1 };
		...
		var bbox3857 = [
			this._map.options.crs.project(this._map.unproject(nwPoint, coords.z)),
			this._map.options.crs.project(this._map.unproject(sePoint, coords.z)),
		];
	...
	_warpBySkew: function (masterTile, finalCanvas) {
		...
		for (var x = -partSize; x <= tileSize.x; x += partSize - 1) {
			for (var y = -partSize; y <= tileSize.y; y += partSize - 1) {
				...
				finalCanvasContext.drawImage(masterTile, out1[0], out1[1], out2[0] - out1[0], out3[1] - out1[1], -partSize / 2, -partSize / 2, partSize, partSize);
			}
		}
	},
	_warpPixelByPixel: function (masterTile, finalCanvas) {
		...
		for (var x = 0; x < finalImageData.width; x++)
			for (var y = 0; y < finalImageData.height; y++) {
				...
				//var out = [x,y];
				if (out[0] < 0 || out[0] > masterImageData.width || out[1] < 0 || out[1] > masterImageData.height) continue;
	...
	_transProj4js: (function () {
		...
		return function (x, y, tilesInfo) {
			...
			let t = proj4(this.t_srs, this.s_srs, p);
			return [(t.x - tilesInfo.shift[0]) / tilesInfo.pixelSize[0], (tilesInfo.shift[1] - t.y) / tilesInfo.pixelSize[1]];
		};
	})(),
});
